...
class ArUcoMarkerDetector:
    def __init__(self):
        ...
        self.highest_marker_id = 8
        # Load the camera calibration data
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.camera_matrix = np.loadtxt(os.path.join(script_dir, 'camera_matrix1.txt'))
        self.dist_coeffs = np.loadtxt(os.path.join(script_dir, 'distortion_coefficients1.txt'))
    ...
    def run(self):
        while True:
            ...
            _, threshold_image = cv2.threshold(gray_image_undistorted, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

            # Detect ArUco markers
            corners, ids, _ = cv2.aruco.detectMarkers(threshold_image, self.aruco_dict, parameters=self.parameters)
